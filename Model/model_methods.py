...
def measure_neural_collapse(self, dl, epoch=-1, log=False):
    ...
    if log is True:
        ...
        rows, cols = torch.triu_indices(mean_angles.shape[0], mean_angles.shape[1])
        class_angles = torch.std(mean_angles[rows, cols]).item()
        class_max_angles = torch.mean((mean_angles[rows,cols]+1)/(self.fc.out_features-1)).item()
        linear_norms = torch.stack(list(weight_norms.values()))
        linear_norms = (torch.std(linear_norms) / torch.mean(linear_norms)).item()
        rows, cols = torch.triu_indices(weight_angles.shape[0], weight_angles.shape[1])
        linear_angles = torch.std(weight_angles[rows, cols]).item()
        linear_max_angles = torch.mean((weight_angles[rows,cols]+1)/(self.fc.out_features-1)).item()
        ...
        self.log['Class Max Angles'].append(class_max_angles)
        self.log['Linear Norms'].append(linear_norms)
        self.log['Linear Angles'].append(linear_angles)
        self.log['Linear Max Angles'].append(linear_max_angles)
